...
class MCP {
  public:
	MCP();									 // default constructor
    MCP(uint8_t address, uint8_t ssPin = 53);// Constructor to instantiate a discrete IC as an object, argument is the address (0-7) of the MCP23S17
	void Init(uint8_t address, uint8_t ssPin = 53);// Constructor to instantiate a discrete IC as an object, argument is the address (0-7) of the MCP23S17
	bool IsValid() const;
	void SetValid();
	void SetNotValid();
	...
	void pinMode_ALL_INPUT();
	void pinMode_ALL_INPUT_PULLUP();
	
	void pinMode_ALL_OUTPUT();
	...
	void interruptMode(unsigned int);        // Selects input state inversion of all I/O pins at once (writing a 1 turns on inversion)
	unsigned int readInterruptSates();		 // Reads the interupt states of all pins
	...
	uint16_t wordRead(uint8_t);              // Reads two registers at once.
  ...
  private:
	...
	uint8_t		 _ssPin;					 // Caches the slave select pin
	static bool _spiInited;
	bool m_isValid;
 };
